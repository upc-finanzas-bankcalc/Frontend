...
export class BonoResultado implements OnInit {
  ...
  loadAllData(bonoId: number): void {
    ...
    this.bonoService.getById(bonoId).subscribe({
      next: (bono: Bono) => {
        ...
        this.loadCashFlow(bonoId);
        this.loadIndicators(bonoId);
      },
  ...
  createCashFlowChart(): void {
    ...
    if (!canvas || !this.cashFlowData || this.cashFlowData.length === 0) {
      return;
    }
      ...
      options: {
        ...
        scales: {
            ...
            y: {
                ...
                title: {
                    display: true,
                    text: `Componentes de Cuota (${this.bondData?.moneda})`
                },
                grid: {
                    display: true,
                    color: 'rgba(0, 0, 0, 0.1)'
